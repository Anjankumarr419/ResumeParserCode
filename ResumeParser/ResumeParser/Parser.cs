...
namespace ResumeParser
{
    public class Parser
    {
        ...
        protected void CheckMarkers(Document doc, ref ArrayList index, string list, ref Hashtable table, int counter)
        {
            ...
            {
                string singlePara = para.GetText().Replace("\r", "").ToLower().Trim(':');//.Replace(" ","");
        ...
        public bool IsValidName(string isName)
        {
            ...
            if (!(string.Equals(isName, "Resume", StringComparison.OrdinalIgnoreCase) || string.Equals(isName, "Curriculum vitae", StringComparison.OrdinalIgnoreCase)))
            {
        ...
        public string IsValidPhone(string phoneNum)
        {
            ...
            {
                if (phoneNum.Contains("s"))
                    phone = Regex.Replace(phoneNum, @"[^0-9]+", "+");
            }
            const string MatchPhonePattern = @"(\+[0-9]{2}|\+[0-9]{2}\(0\)|\(\+[0-9]{2}\)\(0\)|00[0-9]{2}|0)([0-9]{9}|[0-9\-\s]{7,13})";
            Regex rx = new Regex(MatchPhonePattern, RegexOptions.Compiled | RegexOptions.IgnoreCase);
           
            // Find matches.
            MatchCollection matches = rx.Matches(phone == "" ?phoneNum:phone);
